...
Write-Host "█|█| █|  █|█|  █|█| ██|█|  █|█|  █|" -ForegroundColor DarkGreen
Write-Host "█| █|█████|█████|█|  █|█████|█████|" -ForegroundColor DarkGreen
Write-Host "Minecraft Kujnuj instalační skript" -ForegroundColor DarkGreen
Write-Host ""
...
$mods = `
	...
	"https://cdn.modrinth.com/data/6iTJugQR/versions/E9PbJYi2/amendments-1.20-1.2.19-fabric.jar",
...
$scripts = "https://srv.hbmc.net/modpack/kujnuj/scripts.zip"

$configs = "https://srv.hbmc.net/modpack/kujnuj/config.zip"

$cwd = Get-Location
$newest = Invoke-WebRequest $upstream -TimeoutSec 5
$newest = ($newest.ToString() -split "\n")[0]
$modpack = "Žádný"
if (Test-Path ".VERSION") {
	$modpack = ((Get-Content ".VERSION").ToString() -split "\n")[0]
} elseif (Test-Path "INSTALACE.txt") {
	$modpack = "KUJNUJ Manuální Instalace"
} elseif ((Get-ChildItem -Path "mods" -Filter "*.jar" -ErrorAction SilentlyContinue | Measure-Object).Count -gt 0) {
	$modpack = "Neznámý"
}

function Install-FetchFiles {
	param (
		$array,
		$folder
	)
	Write-Host "Otevírám složku $folder"
	...
	foreach ($item in $array) {
		...
		Write-Host "[$i / $($array.Count)] Stahuji $file"
		if (Test-Path $file) {
			Write-Host "[$i / $($array.Count)] Přeskakuji stahování $file"
		} else {
			Invoke-WebRequest -Uri $item -UserAgent $useragent -OutFile $file
		}
		$i = $i + 1
	}
	Write-Host "Opouštím složku $folder"
...
function Install-ExtractFiles {
	...
	Write-Host "Stahuji $webpath"
	Invoke-WebRequest -Uri $webpath -OutFile ".extracted.zip"
	Write-Host "Rozbaluji do $folder"
	Expand-Archive -Path ".extracted.zip" -DestinationPath $folder -ErrorAction SilentlyContinue
	Remove-Item ".extracted.zip" -Force
}

function Install-Modpack {
	if ($modpack -ne "Žádný") {
		Write-Host "Detekován modpack. Ve složce máte módy, které mohou narušit fungování modpacku." -ForegroundColor Red
		Write-Host "Přejete si přesto pokračovat? (Instalátor nebude pokoušet přepisovat soubory)" -ForegroundColor Red
		$yesno = Read-Host "[ano|ne]"
		if ($yesno -ne "ano") {
			Write-Host "Nic neprovedeno"
			return
		}
	}
	Install-FetchFiles $mods "mods"
	Install-FetchFiles $resourcepacks "resourcepacks"
	Install-FetchFiles $shaderpacks "shaderpacks"
	Install-ExtractFiles $configs "."
	Install-ExtractFiles $scripts "."
	Install-FetchFiles $toplevel "."
	if (-Not (Test-Path "options.txt")) {
		Invoke-WebRequest -Uri "https://srv.hbmc.net/modpack/kujnuj/options.txt" -UserAgent $useragent -OutFile "options.txt"
	}

	Write-Host "Vytvářím .VERSION soubor"
	$null = New-Item -ItemType File -Name ".VERSION" -Value $version -Force
	$global:modpack = $version
	Write-Host "Modpack $version úspěšně nainstalován!" -ForegroundColor Green

}

function Uninstall-Modpack {
	Write-Host "Tato akce nejde vrátit zpět!" -ForegroundColor Red
	Write-Host "Opravdu si přejete nadobro vymazat všechny módy?" -ForegroundColor Red
	$yesno = Read-Host "[ano|ne]"
	if ($yesno -eq "ano") {
		Write-Host "Maži mods"
		Remove-Item "mods" -Recurse -ErrorAction SilentlyContinue
		Write-Host "Chcete vymazat i configy?" -ForegroundColor Red
		$yesno = Read-Host "[ano|ne]"
		if ($yesno -eq "ano") {
			Write-Host "Maži config"
			Remove-Item "config" -Recurse -ErrorAction SilentlyContinue
			Remove-Item "options.txt" -ErrorAction SilentlyContinue
		}
		Write-Host "Maži instrukce"
		Remove-Item "INSTALACE.txt" -ErrorAction SilentlyContinue
		Remove-Item ".VERSION" -Force -ErrorAction SilentlyContinue
		$global:modpack = "Žádný"
		Write-Host "Úspěšně vymazáno"
	} else {
		Write-Host "Nic neprovedeno"
	}
}

function Test-Diagnose {
	param (
		$verbose
	)
	Write-Host "Verze Skriptu: " -NoNewline -ForegroundColor Green
	Write-Host $version
	Write-Host "Detekovaný Modpack: " -NoNewline -ForegroundColor Green
	Write-Host $modpack
	Write-Host "Nejnovější Verze: " -NoNewline -ForegroundColor Green
	Write-Host $newest
	Write-Host "Instalační Cesta: " -NoNewline -ForegroundColor Green
	Write-Host $pwd
	if ($verbose) {
		Write-Host "Počet modů v mods složce: " -NoNewline -ForegroundColor Green
		Write-Host (Get-ChildItem -Path "mods" -Filter "*.jar" -ErrorAction SilentlyContinue | Measure-Object).Count
		Write-Host "Počet modů v modpacku: " -NoNewline -ForegroundColor Green
		Write-Host $mods.Count
		Write-Host "Upstream: " -NoNewline -ForegroundColor Green
		Write-Host $upstream
	}
	Write-Host ""
	if ($cwd -notlike "*.minecraft") {
		Write-Host "Tato složka není '.minecraft', ujistěte se, že jste skript opravdu spustili v minecraft složce" -ForegroundColor Red
	}
	if ($modpack -ne "Žádný") {
		Write-Host "Detekován modpack. Při instalaci je nutno tuto verzi nejprve odinstalovat." -ForegroundColor DarkYellow
	}
	if ($version -ne $newest) {
		Write-Host "Dostupná nová verze $newest!" -ForegroundColor DarkYellow
	}
}
...
while ($alive) {
	Write-Host ""
	Write-Host "Dostupné akce:"
	Write-Host "    1 - Nainstalovat Modpack"
	Write-Host "    2 - Odinstalovat/Vyčistit Modpack"
	Write-Host "    3 - Diagnostika"
	Write-Host "    4 - Odejít"
	Write-Host ""
	$userinput = Read-Host -Prompt "Akce"
	Write-Host ""
	...
	switch ($userinput) {
		...
		"4" {
			$alive = $false
			Write-Host "[Konec]" -ForegroundColor Green
		}
	}
}
